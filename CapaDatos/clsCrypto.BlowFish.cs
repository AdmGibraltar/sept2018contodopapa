    ...
    public class BlowFish {
        public static string Encrypt(string stringToEncrypt, string sKey) {
            ...
            try {
                string strTmp = String.Format(
                    "{0}|@|{1}", 
                    clsCrypto.MD5.MD5Hash(stringToEncrypt),
                    stringToEncrypt
                );

                BlowFishCS b = new BlowFishCS(StringToHex(sKey));
                cipherText = b.Encrypt_CBC(strTmp);
        ...
        public static string Decrypt(string stringToDecrypt, string sKey) {
            ...
            try 
            {
                ...
                string Dec = b.Decrypt_CBC(stringToDecrypt);
                string[] strTmp = Dec.Split(stringSeparators, StringSplitOptions.None);

                if (strTmp.Length == 2)
                {
                    if (clsCrypto.MD5.MD5ValidHash(strTmp[1], strTmp[0]))
                    {
                        Result = strTmp[1];
                    }
                    else
        ...
        /// <param name="hexKey">Cipher key as a hex string</param>
                            public BlowFishCS(string hexKey)
        {
        ...
        /// <param name="cipherKey">Cipher key as a byte array</param>
                            public BlowFishCS(byte[] cipherKey)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(cipherKey);
        }
